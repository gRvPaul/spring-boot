node('master') {
	...
	stage('Create Dockerfile from template') {
		sh '''
			...
			then
				dockerfile-template -f $DOCKER_TEMPLATE_PATH/Dockerfile.template_$APP_TYPE -d BASE=java -d TAG=8 -d PORT=8080 -d PROJECT_OUTPUT=target/spring-boot.jar > $DOCKERFILE_PATH
